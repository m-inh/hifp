...
short int wave16[NUMWAVE*num_songs];
short int fpid[NUMDWTECO*num_songs];
...
unsigned int c_fpid[128*num_songs]; // compress fpid
...
int main(int argc, char ** argv)
{
    ...
    while ((ep = readdir(dir)) != NULL && song_id < MAX_SONGS && song_id < num_songs)
    {
        if (ep->d_type == DT_REG)
        {
            ...
            short int chunk_wave[NUMWAVE];
            memset(chunk_wave, 0, sizeof(chunk_wave));

            wave_header = read_wave_header(ifp);
            read_wav_data(ifp, chunk_wave, wave_header);

            int wave_offset = song_id * NUMWAVE;

            for (int i=0; i<NUMWAVE; i++) {
                wave16[wave_offset+i] = chunk_wave[i];
            }
...
void run()
{
    ...
    /* Create buffer */
    wave16_buf = clCreateBuffer(context, CL_MEM_READ_ONLY, (NUMWAVE * num_songs) * sizeof(short int), NULL, &status);
    checkError(status, "Failed to create buffer for input");
    fpid_buf   = clCreateBuffer(context, CL_MEM_READ_WRITE, (NUMDWTECO * num_songs) * sizeof(short int), NULL, &status);
    ...
    status = clEnqueueWriteBuffer(queue, wave16_buf, CL_FALSE, 0, (NUMWAVE * num_songs) * sizeof(short int), wave16, 0, NULL, &write_event[0]);
    ...
    status = clEnqueueReadBuffer(queue, fpid_buf, CL_FALSE, 0, (NUMDWTECO * num_songs) * sizeof(short int), fpid, 1, &kernel_event[0], &read_event[0]);
